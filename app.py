...
# --- 2. PLAN DE ESTUDIOS Y CRONOGRAMA ---
PLAN_ESTUDIOS = {
...
CRONOGRAMA = {
    "Lunes": "Macroeconomía (18:00 - 22:00)",
    "Martes": "Control de Gestión (18:00 - 22:00)",
    "Miércoles": "Libre - ¡A estudiar resúmenes!",
    "Jueves": "Comercialización (18:00 - 22:00)",
    "Viernes": "Taller de Integración (18:00 - 22:00)"
}
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("<p class='hp-bar-text'>RECURSOS:</p>", unsafe_allow_html=True)
        st.link_button("📚 RESÚMENES (Drive)", "https://google.com") # Cambia por tu link real
        st.link_button("🏫 CAMPUS VIRTUAL", "https://campus.unla.edu.ar/")
    ...
    if st.session_state.menu == "Inicio":
        # EASTER EGGS DE BIENVENIDA
        if n_aprobadas >= 42: st.balloons(); st.success("🎖️ ¡MISIÓN CUMPLIDA! ERES LICENCIADO/A!")
        elif n_aprobadas > 0: st.toast(f"¡Soldado {usuario} reportándose! {n_aprobadas} victorias confirmadas.")
        ...
        with col_av:
            ...
            # CRONOGRAMA SEMANAL
            st.markdown("---")
            st.markdown("<p class='hp-bar-text'>📅 CRONOGRAMA:</p>", unsafe_allow_html=True)
            for dia, materia in CRONOGRAMA.items():
                st.markdown(f"**{dia}:** {materia}")
        
        with col_cur:
            st.metric("AVANCE DE GUERRA", f"{int((n_aprobadas/42)*100)}%")
            st.progress(n_aprobadas/42)
            st.markdown("#### ⚔️ OBJETIVOS ACTUALES:")
